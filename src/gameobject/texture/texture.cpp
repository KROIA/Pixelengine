...
{
    TEXTURE_FUNCTION(profiler::colors::Brown);
...
Texture &Texture::operator=(const Texture &other)
{
    ...
    this->m_originType      = other.m_originType;
    TEXTURE_BLOCK("m_texture",profiler::colors::Brown);
    this->m_texture         = other.m_texture;
    TEXTURE_END_BLOCK;
    this->m_rotation        = other.m_rotation;
    return *this;
...
{
    TEXTURE_FUNCTION(profiler::colors::Brown100);
...
{
    TEXTURE_FUNCTION(profiler::colors::Cyan700);
    switch(m_originType)
    {
        case Origin::topLeft:
...
{
    TEXTURE_FUNCTION(profiler::colors::Brown200);
...
{
    TEXTURE_FUNCTION(profiler::colors::Brown50);
    ...
    TEXTURE_BLOCK("fillPixelPos x loop",profiler::colors::Brown400);
    for(unsigned int x=0; x<texture_size.x; x++)
    {
        TEXTURE_BLOCK("fillPixelPos y loop",profiler::colors::Brown500);
    ...
    TEXTURE_BLOCK("Fill rawRectList",profiler::colors::Brown100);
    ...
    TEXTURE_BLOCK("Read max texture size",profiler::colors::Brown200);
    ...
    TEXTURE_BLOCK("Fill map with nullptr type: RectI*",profiler::colors::Brown300);
    ...
    TEXTURE_BLOCK("Fill map with new RectI*",profiler::colors::Brown400);
    ...
    TEXTURE_BLOCK("Move m_pixelRectList",profiler::colors::Brown500);
    ...
    TEXTURE_BLOCK("Delete map",profiler::colors::Brown600);
    for(size_t x=0; x<width; x++)
    {
        for(size_t y=0; y<height; y++)
        {
...
void Texture::optimize_HitboxMap(vector<vector<RectI*>  > &map,vector<RectI> &outputColliderList)
{
    ...
    TEXTURE_BLOCK("Optimizer y loop",profiler::colors::Brown100);
    for(size_t y=0; y<height; y++)
    {
        TEXTURE_BLOCK("Optimizer x loop",profiler::colors::Brown200);
        for(size_t x=0; x<width; x++)
        {
            if(map[x][y] == nullptr)
                continue;
            ...
            TEXTURE_BLOCK("while(!endXloop)",profiler::colors::Brown300);
            while(!endXloop)
            {
            ...
            TEXTURE_END_BLOCK;
            TEXTURE_BLOCK("Delete unused Rects",profiler::colors::Brown300);
